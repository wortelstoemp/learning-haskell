...
-- Haskell is statically typed, uses type inference.
...
-- ---------- From "Don't Fear The Monad" ----------
-- Build complexity by combining simple functions.
...
-- f: a -> a
-- g: b -> a
-- Composition of functions (types need to line up)
-- f ° g = h
-- h: a -> a
-- Associativity: (f ° g) ° h = f ° (g ° h)

-- Monoid
-- Is a collection of things + a rule of combining the things
-- Rule follows these 2 rules:
--     All rules are associative but not necessarily commutative
--     rule x e = e (with e the unit)
-- Example has following monoid:
-- Numbers of clock [1, 12] + "rule = mod (x + y) 12" (e = 12)

-- Functions under composition form a monoid.

-- Monad
-- Almost the same as monoid.
-- f: a -> M a          (M is another function)
-- g: a -> M a
-- h: a -> M a
...
modEx2 = 5 `mod` 4 -- `mod` is an infix operator
...
-- Comparison Operators : < > <= >= == /= (not equals to)
